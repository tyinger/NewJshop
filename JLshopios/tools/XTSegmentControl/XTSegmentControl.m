...
#define kSeletedColor [UIColor redColor]
#define kNormolColor [UIColor darkGrayColor]
...
//                [_titleIconView addRoundBorder];
                _titleIconView.layer.masksToBounds = YES;
                _titleIconView.layer.cornerRadius = 5;
                if (title) {
...
- (void)setSelected:(BOOL)selected
{
    switch (_type) {
        ...
        {
            if (_titleLabel) {
                [_titleLabel setTextColor:(selected ? kSeletedColor /*[UIColor colorFromHexString:@"0x3bbd79"]*/:kNormolColor)];
...
- (void)doTap:(UITapGestureRecognizer *)sender
{
    ...
    [_itemFrames enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            [weakSelf selectIndex:idx isAction:YES];
...
- (void)initItemsWithTitleArray:(NSArray *)titleArray withIcon:(BOOL)isIcon
{
    ...
    if ([obj isKindOfClass:[NSString class]]) {
        for (int i = 0; i < titleArray.count; i++) {
            ...
            float width = CGRectGetWidth(self.bounds)/titleArray.count;
...
- (void)addRedLine
{
    if (!_lineView) {
        ...
        _lineView.backgroundColor = kSeletedColor /*[UIColor colorFromHexString:@"0x3bbd79"]*/;
        ...
        bottomLineView.backgroundColor = [UIColor colorWithRed:200/255.0 green:199/255.0 blue:200/255.0 alpha:1];
...
{
    [self selectIndex:index isAction:NO];
}

- (void)selectIndex:(NSInteger)index isAction:(BOOL)isTransformAction
{
    if (isTransformAction) {
        if (self.shouldSelectedItemBlock) {
            if (!self.shouldSelectedItemBlock(index)) {
                return;
            }
        }
    }
        ...
        if (index != _currentIndex) {
            ...
            if (_currentIndex < 0) {
                ...
                [UIView animateWithDuration:XTSegmentControlAnimationTime animations:^{
                    ...
                    _currentIndex = index;
                    if (isTransformAction) {
                        [weakSelf transformAction:index];
                    }
                }];
            }
        }
...
{
    if (_contentView.contentSize.width <= CGRectGetWidth(self.bounds)) {
        return;
    }
